...
The console application requires RDMFieldDictionary and enumtype.def to decode the raw data in Hex string format to the actual value. The user can update dictionary inside folder Dict to the latest version. In case that the application unable to find field definition of the fid and enum value, it will instead write original Hex string in the tracing log. Note that you can download latest version of data dictionary from the [Software Download page](https://customers.thomsonreuters.com/a/support/technical/softwaredownload/default.aspx) and then search for product name "TREP Template service pack".
...
Start command prompt and change folder to "<GitHub Repo>\Example.EMARFACPP.Tool.RSSLTraceViewer\rsslxmltracedataconverter", you should see rsslxmltracedataconverter.csproj in the folder. Then run command

```
dotnet build
```

To build the project or just type __"dotnet run"__ to run the application. It will automatically compile and build the project and run the application. It show the following console output after running __dotnet run__.
...
Based on output from previous section, it required one argument which is "-s" following by name or path to RSSL Trace file you want to convert. You can also specify XML output file by using -o following by output file name. If user do not set output file name, the application will set it to "output.xml" instead. Note that "-r" and "-e" is optional command line argument to specify path to your own data dictionary. By default it will read the dictionary from folder "<RunningDirectory>\Dict". Please make sure that you have folder "Dict" in your running directory.
...
```
There are additional command line option "--verbose" which can print output to console while the utiltlis processing the data. Below is the sample output.

As described in Overview section, the applicaiton can be used to decoded data for Level 2 data and below sample is an output for the Market By Order domain when using the application with above sample trace file.
